...
def duplicate_remover():
    ...
    try:
        ...
        # Log operation start
        op_logger.log_start(filename=f.filename, file_size_mb=get_file_size_mb(f.stream) if hasattr(f, 'stream') else 0)
        ...
        try:
            ...
            # Read file into memory before temp_dir cleanup
            with open(out_path, 'rb') as output_file:
                file_data = output_file.read()
            
            # Choose a sensible mimetype for the returned file
            if ext == 'csv':
                mime = 'text/csv'
            elif ext in ('xlsx', 'xls'):
