...
def configure_database_pooling(app, db):
    ...
    # Setup event listeners on the db instance
    setup_event_listeners(db)
    setup_slow_query_logging(app, db)
    
    return app

def setup_event_listeners(db):
...
def setup_slow_query_logging(app, db, threshold_ms=1000):
...
class SlowQueryLogger:
    """Log slow database queries (legacy)."""
    pass
