...
class AuthManager:
    ...
    def create_user(username, email, password):
        ...
        try:
            ...
            # If a user with same username/email already exists, return its id and api_key
            try:
                cursor.execute('SELECT id, api_key FROM users WHERE username=? OR email=?', (username, email))
                row = cursor.fetchone()
                if row:
