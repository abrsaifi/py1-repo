...
Minimal PDF/image processing utilities wrapped as a package.
...
```bash
python -m app.cli input.pdf output_bw.pdf --dpi 300 --threshold 128
```

Or run the thin top-level launcher:
...
- Tests live in the repository as simple scripts; CI runs `pytest`.
