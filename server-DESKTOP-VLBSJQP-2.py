...
def upload_chunk():
    ...
    try:
        ...
        if expected_total and index >= expected_total - 1:
            # assemble
            assembled_path = os.path.join(upload_dir, sanitize_filename(filename) or secure_filename(filename))
...
def powerpoint_to_pdf(pptx_path, output_pdf):
    ...
    try:
        ...
        for slide_num, slide in enumerate(prs.slides, 1):
            ...
            # Draw slide number and content
            y = letter[1] - 40
            c.drawString(40, y, f"--- Slide {slide_num} ---")
            y -= 20
...
def preview_conversion():
    ...
    Accepts form-data with file(s) and optional `operation` (e.g., 'bw', 'to-pdf', 'image-convert').
    Returns JSON: { success: bool, image: dataurl, message: '' }
    """
    ...
    try:
        ...
        if ext == 'pdf':
            ...
            ok = False
            if ext == 'docx':
                ok = docx_to_pdf(input_path, inter)
            elif ext == 'doc':
                ok = soffice_to_pdf(input_path, inter)
            if ok and os.path.exists(inter):
                source_pdf = inter
        elif ext in EXCEL_ALLOWED_EXTENSIONS:
            inter = os.path.join(temp_dir, f"__preview_{Path(filename).stem}.pdf")
            ok = excel_to_pdf(input_path, inter)
            if ok and os.path.exists(inter):
                source_pdf = inter
        ...
        # If we have a PDF (either uploaded or converted), render first page
        if source_pdf:
            ...
            if img_bytes_list:
                ...
                return jsonify({'success': False, 'message': 'Could not render preview from PDF'}), 500
...
def html_to_pdf(html_content, output_pdf):
    ...
    try:
        ...
        # Simple HTML parsing - just convert paragraphs
...
def text_to_pdf(text_content, output_pdf, font_size=11):
...
def api_users_list():
    """Get list of all users (mock data)"""
    try:
        users = [
            {
                'id': '1',
